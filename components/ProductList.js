...
const ProductList = ({ product, setSelectedProduct, setIsEditModalOpen, setViewopen, setIsQuantityOpen, fetchData, setNavigating }) => {
    ...
    <tr key={product._id}>
      <td className="border-b sm:px-4 sm:py-2 px-2 py-1 whitespace-nowrap font-bold capitalize cursor-pointer"
        onClick={() => {
          setNavigating(true);
          setTimeout(() => {
            router.push(`addproduct/${product._id}?name=${encodeURIComponent(product.productName)}`);
          }, 300);
        }}
